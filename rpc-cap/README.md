...
Once this middleware is added to a server that is directing requests to a [JsonRpcEngine](https://github.com/onekeyhq/json-rpc-engine), it adds some new methods to the RPC server:
...
The capability system is initialized with a variety of options, and is itself a [@onekeyhq/controllers](https://github.com/onekeyhq/extension-controllers/)-compatible controller, which defines how new permissions are persisted to state, and how they are restored in the constructor.

Once initialized, it exposes a special [AuthenticatedJsonRpcMiddleware](https://github.com/onekeyhq/rpc-cap/blob/master/src/%40types/index.d.ts#L7-L15) type method `providerMiddlewareFunction(domainInfo, req, res, next, end)`, which requires an assumed-authenticated `domain` object, followed by normal `json-rpc-engine` middleware parameters.
...
Our persistence model depends on the `@onekeyhq/controllers` architecture, which means state updates are emitted via `controller.subscribe((update) => {/* handle update */});`
...
To test against an example dapp, serve the example using `yarn serve` and explore using [this branch of Onekey](https://github.com/onekeyhq/onekey-extension/tree/LoginPerSite).
